...
const Checkout = function () {
  ...
  const paymentSubmit = () => {
    if (
      ...
      const orderData = {
        ...
        shipping,
        discountPrice,
        shippingAddress, // this is above object
        user,
      };
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.get(`${base_url}coupon/get-coupon-name/${name}`).then((res) => {
      const shopId = res?.data?.couponeCode?.shopId;
      const copounCodeValue = res.data.couponeCode?.value;
      if (res.data.couponCode !== null) {
        ...
        if (isCouponValid.length === 0) {
          ...
          const discountPrice = (eligiblePrice * copounCodeValue) / 100;

          setDiscountPrice(discountPrice);
          setcouponCodeData(res.data?.couponeCode?.name);
          setCouponCode("");
        }
      }
